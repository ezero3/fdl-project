...
## Step 2b. Clarify scope (if needed)

Before switching branches or writing the spec, assess the complexity of the feature.

If the feature seems broad, vague, or likely to have unclear boundaries (e.g. involves multiple user-facing flows, touches several system layers, or the description is ambiguous), ask the user up to 5 clarifying questions **before proceeding**. Focus on:

- What is explicitly out of scope?
- Suggest sub-features that could be separate specs?
- Any constraints on platform or integration?
- Identify potential pitfalls?

Wait for the user's answers, then incorporate them into the spec. If the feature is small and well-defined, skip this step entirely and proceed.

## Step 3. Switch to a new Git branch

Before making any content, switch to a new Git branch using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `feature/google-api-01`
